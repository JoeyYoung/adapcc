...
int main(int argc, char *argv[]){
    ...
    /*
        4GPU, GPU2 GPU3 (187450375) + GPU2 GPU3 (187450376);
    */
    int offset = 2;
    int gpuId = (localRank + offset) % 4;
    cudaSetDevice(gpuId);
    ...
    // persistent - g16 gpu2 receive flow
    if(gpuId == 2 && int(hostHashs[myRank])==187450375){
        while(1){
            MPI_Recv(recvTensorDevBuffer, size, MPI_FLOAT, 2, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
    }

    // persistent - g17 gpu2 send flow
    if(gpuId == 2 && int(hostHashs[myRank])==187450376){
        while(1){
            MPI_Send(localTensorDevBuffer, size, MPI_FLOAT, 0, 2, MPI_COMM_WORLD); // tag 2
        }
    }
    ...
    start = clock();
    // g16 gpu3 send flow
    if(gpuId == 3 && int(hostHashs[myRank])==187450375){
        MPI_Send(localTensorDevBuffer, size/10, MPI_FLOAT, 3, 3, MPI_COMM_WORLD); // tag 3
    }

    if(gpuId == 3 && int(hostHashs[myRank])==187450376){
        MPI_Recv(recvTensorDevBuffer, size/10, MPI_FLOAT, 1, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
